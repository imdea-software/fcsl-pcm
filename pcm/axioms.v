...
by move: (pf); rewrite pf in w H * => {pf}-pf; rewrite !eqc. Qed.

Lemma jmE A (v w : interp A) : jmeq v w <-> v = w.
Proof.
by split=> [/(_ erefl)//|->]; apply: jmrefl. Qed.

Lemma castE A B (pf1 pf2 : A = B) (v1 v2 : interp B) :
  v1 = v2 <-> cast pf1 v1 = cast pf2 v2.
Proof.
by move: (pf1) pf2; rewrite pf1=> *; rewrite !eqc. Qed.
...
Proof.
by rewrite -pf in w *; rewrite jmE; split=> [->|/dyn_inj]. Qed.
