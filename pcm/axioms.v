...
Lemma p1_eq (P : Prop) (p1 p2 : P) : p1 = p2.
Proof. by apply/ext_prop_dep_proof_irrel_cic/pext. Qed.

Lemma fmorph_Type A (B : A -> Type) (f : forall x, B x) : f = [eta f].
Proof. by apply: fext. Qed.   

Lemma injective_inj A P : injective (@sval A P).
Proof.
move=>[x Hx][y Hy] /= H; move: Hx Hy; rewrite H=>*. 
congr exist; apply: pf_irr.
Qed.

Lemma proj_e1 A (P : A -> Prop) x H : sval (exist P x H) = x.
Proof. by []. Qed.

Lemma comp_id A B (f : A -> B) : f = f \o id.
Proof. by apply: fext. Qed.

Lemma comp_id A B (f : A -> B) : f = id \o f.
Proof. by apply: fext. Qed.
...
Lemma jmeq_interp (A B : Type) interp (pf : A = B) (v : interp A) (w : interp B) :
