...
Lemma joinmK m1 m2 m :
  valid (m1 \+ m) -> m1 \+ m = m2 \+ m -> m1 = m2.
...
Definition mutex_eq (x y : mutex T) :=
  match x, y with undef, undef => true
  | nown, nown => true
  | own, own => true
  | mx x', mx y' => x' == y'
  | _, _ => false
...
by case: eqP => [->|H]; constructor=> //; case=> /H. Qed.
...
Lemma mxxMy t x y :
  valid (x \+ (mx t \+ y)) -> x \+ y = Unit.
Proof. by rewrite joinCA => /mxMx. Qed.

Lemma mxyMx t x y :
  valid (y \+ mx t \+ x) -> y \+ x = Unit.
...
Lemma mxE2 t :
  ...
  ((nown == mx t) = false) *
  ((mx t == mx_undef) = false) *
  ((mx_undef == mx t) = false) * ((mx t == own) = false) * ((own == mx t) = false).
Proof. by []. Qed.
...
Lemma mxE3 t x :
  ...
  ((nown == mx t \+ x) = false) *
  ((nown == x \+ mx t) = false) *
  ((mx t \+ x == own) = false) * ((x \+ mx t == own) = false) * ((own \+ x == mx t) = false) * ((x \+ own == mx t) = false).
Proof. by case: x. Qed.

Lemma mxE4 x :
  ((own \+ x == nown) = false) *
  ((x \+ own == nown) = false) *
  ((own \+ x == own) = (x == Unit)) * ((x \+ own == own) = (x == Unit)).
Proof. by case: x. Qed.
...
by do !split=> //; rewrite ?L // eq_sym L eq_sym. Qed.
...
Definition mxE := (mxE1, mxE2, mxE3, mxE4, mxE5, mx_injE,
   mx_valid, mx_valid_own, (* plus a bunch of safe simplifications *) unitL, unitR, valid_unit, eq_refl, valid_undef, undef_join, join_undef).
