...
Record mixin_of (T : Type) := Mixin {
  ...
  ojoinA_op : forall x y z, obind (ojoin_op x) (ojoin_op y z) = obind (ojoin_op^~ z) (ojoin_op x y);
...
Proof.
by case: U x => T[ou oj ojC ojA ojL]. Qed.
End Exports.
...
Let join := [fun x y : lift A => if (x, y) is (up v, up w) then
      if ojoin v w is Some u then up u else @down A else @down A].
...
case E2: (ojoin x y)=> [v2|] //. by move: (ojoinA x y z); rewrite E1 E2 /= => <-. Qed.
...
Proof.
case=> [|x] [|y] /=; try by constructor. by apply: (iffP eqP)=> [->|[]]. Qed.
...
Definition liftPCMMixin :=
  PCMMixin (@Lift.joinC A) (@Lift.joinA A) (@Lift.unitL A) (@Lift.validL A) (@Lift.validU A).
...
Lemma nxV (m1 m2 : lift natUnlifted) :
  valid (m1 \+ m2) ->
  exists n1 n2, m1 = up n1 /\ m2 = up n2.
Proof.
by case: m1 => //n1; case: m2 => // n2; exists n1, n2. Qed.

Lemma nxE0 (n1 n2 : lift natUnlifted) :
  n1 \+ n2 = up 0 -> (n1 = up 0) * (n2 = up 0).
Proof.
case: n1 n2 => [|n1] [|n2] //; rewrite upE /ojoin /=.
by case=> /eqP; rewrite addn_eq0 => /andP[/eqP-> /eqP->]. Qed.
