...
Notation "[ 'Pred' x y | E ]" :=
  (SimplPred (fun t => let: (x, y) := t in E))
  (at level 0, x ident, y ident,
   format "[ 'Pred'  x  y  |  E ]") : fun_scope.
...
Definition EqSimplPred T (p1 p2 : Simpl_Pred T) :=
  EqPredType p1 p2.

Definition SubSimplPred T (p1 p2 : Simpl_Pred T) :=
  SubPredType p1 p2.
...
Notation "[ 'Pred' x y \In A & B ]" :=
  [Pred x y | (x \In A) /\ (y \In B)]
  (at level 0, x ident, y ident,
...
Proof.
  by rewrite /Mem; case: pT => T1 app1 [mem1 /= ->]. Qed.
...
Proof.
by move=> H1 H2 x; split; [move/H1; move/H2 | move/H2; move/H1]. Qed.

Lemma SubPredType_trans' (r1 r2 r3 : pT) :
  SubPredType r1 r2 -> SubPredType r2 r3 -> SubPredType r1 r3.
Proof.
by move=> H1 H2 x; move/H1; move/H2. Qed.
...
Add Parametric Relation T (pT : PredType T) :
  pT (@EqPredType _ pT) reflexivity proved by (@EqPredType_refl _ _) symmetry proved by (@EqPredType_sym _ _) transitivity proved by (@EqPredType_trans' _ _) as EqPredType_rel.
...
Add Parametric Morphism
  T (pT : PredType T) :
  (@EqPredType T pT) with
  signature
  @EqPredType _ _ ==> @EqPredType _ _ ==> iff as EqPredType_morph.
Proof.
by move=> r1 s1 H1 r2 s2 H2; rewrite H1 H2. Qed.

Add Parametric Morphism
  T (pT : PredType T) :
  (@SubPredType T pT) with
  signature
  @EqPredType _ _ ==> @EqPredType _ _ ==> iff as SubPred_morph.
Proof.
by move=> r1 s1 H1 r2 s2 H2; split=> H x; move/H1; move/H; move/H2. Qed.
...
Lemma orrC (r1 r2 : Pred T) : r1 +pr2 <~> r2 +p r1.
Proof.
move=> x; split=> /=; tauto. Qed.
...
Lemma orrCA (r1 r2 r3 : Pred T) :
  r1 +p r2 +p r3 <~> r2 +p r1 +p r3.
Proof. by move=> x; split=> /=; intuition. Qed.

Lemma orrAC (r1 r2 r3 : Pred T) :
  (r1 +p r2) +p r3 <~> (r1 +p r3) +p r2.
Proof. by move=> ?; split=> /=; intuition. Qed.

Lemma orrA (r1 r2 r3 : Pred T) :
  (r1 +p r2) +p r3 <~> r1 +p r2 +p r3.
Proof.
by rewrite (orrC r2) orrCA orrC. Qed.

(* absorption *)
Lemma orrAb (r1 a : Pred T) :
  r1 <~> r1 +p a <-> a ~> r1.
Proof.
split; first by move=> -> x /=; auto.
move=> H x /=; split; first by auto. by case=> //; move/H. Qed.
...
Lemma subp_asym (p1 p2 : Pred T) :
  p1 <=p p2 -> p2 <=p p1 -> p1 =p p2.
Proof.
by move=> H1 H2 x; split; [move/H1 | move/H2]. Qed.

Lemma subp_trans (p2 p1 p3 : Pred T) :
  p1 <=p p2 -> p2 <=p p3 -> p1 <=p p3.
Proof.
by move=> H1 H2 x; move/H1; move/H2. Qed.

Lemma subp_or (p1 p2 q : Pred T) :
  p1 <=p q /\ p2 <=p q <-> p1 +p p2 <=p q.
Proof.
split=> [[H1] H2 x|H1]; first by case; [move/H1 | move/H2].
by split=> x H2; apply: H1; [left | right]. Qed.

Lemma subp_and (p1 p2 q : Pred T) :
  q <=p p1 /\ q <=p p2 <-> q <=p p1 *p p2.
Proof.
split=> [[H1] H2 x|] H; last by split=> x; case/H. by split; [apply: H1 | apply: H2]. Qed.

Lemma subp_orl (p1 p2 q : Pred T) :
  p1 <=p p2 -> p1 +p q <=p p2 +p q.
Proof.
by move=> H x; case; [move/H; left|right]. Qed.

Lemma subp_orr (p1 p2 q : Pred T) :
  p1 <=p p2 ->
  q +p p1 <=p q +p p2.
Proof.
by move=> H x; case; [left | move/H; right]. Qed.

Lemma subp_andl (p1 p2 q : Pred T) :
  p1 <=p p2 -> p1 *p q <=p p2 *p q.
Proof.
by by move=> H x [H1 H2]; split; [apply: H|]. Qed.

Lemma subp_andr (p1 p2 q : Pred T) :
  p1 <=p p2 ->
  q *p p1 <=p q *p p2.
Proof.
by move=> H x [H1 H2]; split; [|apply: H]. Qed.
...
Fixpoint Mem_Seq (s : seq T) :=
  if s is y :: s' then (fun x => x = y \/ Mem_Seq s' x)
  else xPred0.
...
case; first by move=> ->; exists y; split=> //; left.
  by case/IH=> z [->]; exists z; split=> //; right. Qed.
...
Notation "[ 'Image' E | i : T <- s & C ]" := [Image E | i : T <- [PredI s & C]]
  (at level 0, E at level 99, i ident, only parsing) : rel_scope.

Lemma Image_mem A B (f : A -> B) (P : Pred A) x :
  x \In P -> f x \In Image f P.
