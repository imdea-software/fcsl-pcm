...
Record mixin_of (T : Type) :=
  ...
  _ : forall x y, valid_op (join_op x y) ->
  valid_op x;
    (* unit is valid *)
  _ : valid_op unit_op}.
...
Definition clone :=
  fun c & cT -> T & phant_id (pack c) cT => pack c.
...
Proof.
by case: U x y => tp [v j z Cj *]; apply: Cj. Qed.

Lemma joinA (x y z : U) : x \+ (y \+ z) = x \+ y \+ z.
Proof.
by case: U x y z => tp [v j z Cj Aj *]; apply: Aj. Qed.
...
Proof.
by rewrite joinA (joinC x) -joinA. Qed.

Lemma validL (x y : U) : valid (x \+ y) -> valid x.
Proof.
case: U x y => tp [v j z Cj Aj Uj /= Mj inv f ?]; apply: Mj. Qed.
...
Proof.
by case: U x => tp [v j z Cj Aj Uj *]; apply: Uj. Qed.
...
Proof.
by case: U => tp [v j z Cj Aj Uj Vm Vu *]. Qed.

(* some helper lemmas for easier extraction of validity claims *)
Lemma validAR (x y z : U) :
  valid (x \+ y \+ z) -> valid (y \+ z).
Proof. by rewrite -joinA; apply: validR. Qed.

Lemma validAL (x y z : U) :
  valid (x \+ (y \+ z)) ->
  valid (x \+ y).
...
Definition precise (U : pcm) (P : U -> Prop) :=
  forall s1 s2 t1 t2, valid (s1 \+ t1) -> P s1 -> P s2 -> s1 \+ t1 = s2 \+ t2 -> s1 = s2.
...
by rewrite !(cancPL H V H1 H2 E). Qed.
...
Proof.
case: U x y => V[b] [u1 u2] H1 H2 H3 T x y; split; first by apply: H2. by case=> -> ->; rewrite unitL. Qed.
...
Proof.
  by rewrite (negbTE valid_undefN). Qed.
...
Canonical pcm_monoid (U : pcm) :=
  Monoid.Law (@joinA U) (@unitL U) (@unitR U).
...
Lemma big_pmorph I r (P : pred I) F :
  ...
  f (\big[PCM.join/Unit]_(i <- r | P i) F i) = \big[op1/id1]_(i <- r | P i) f (F i).
Proof.
rewrite unlock; elim: r=> [|x r IH] //=; case: ifP => //H V.
  by rewrite f_op // IH //; apply: validR V. Qed.
...
Definition pjoin :=
  [fun x1 x2 : tp => (x1.1 \+ x2.1, x1.2 \+ x2.2)].

Definition punit : tp := (Unit, Unit).

Lemma joinC x y : pjoin x y = pjoin y x.
Proof.
move: x y => [x1 x2] [y1 y2] /=. by rewrite (joinC x1) (joinC x2). Qed.

Lemma joinA x y z : pjoin x (pjoin y z) = pjoin (pjoin x y) z.
Proof.
move: x y z => [x1 x2] [y1 y2] [z1 z2] /=.
by rewrite (joinA x1) (joinA x2). Qed.

Lemma validL x y : pvalid (pjoin x y) -> pvalid x.
Proof.
move: x y => [x1 x2] [y1 y2] /=. by case/andP=> D1 D2; rewrite (validL D1) (validL D2). Qed.
...
Canonical prodPCM U V :=
  Eval hnf in PCM (_ * _) (@prodPCMMixin U V).
...
by case=> X1 X2; elim: H; rewrite X1 X2 !unitbE. Qed.
...
Proof.
by case: x y => x1 x2 [y1 y2] [] /joinE0[-> ->] /joinE0[-> ->]. Qed.

Lemma prod_valid_undef : ~~ valid (@undef U, @undef V).
Proof. by rewrite pcmPV negb_and !valid_undef. Qed.

Lemma prod_undef_join x :
  (@undef U, @undef V) \+ x = (@undef U, @undef V).
...
Definition boolPCMMixin :=
  PCMMixin andbC andbA BoolConjPCM.unitL (fun x y => @id true) (erefl _).
...
Proof.
by exists Unit; rewrite unitR. Qed.

Lemma pleq_trans x y z : [pcm x <= y] -> [pcm y <= z] -> [pcm x <= z].
Proof.
by case=> a -> [b ->]; exists (a \+ b); rewrite joinA. Qed.
...
Lemma pleq_joinxK (V : cpcm) (x x1 x2 : V) :
  valid (x2 \+ x) ->
  [pcm x1 \+ x <= x2 \+ x] -> [pcm x1 <= x2].
Proof.
by move=> W [a]; rewrite joinAC=> /(joinKx W)->; exists a. Qed.
...
Definition local (U : pcm) (f : U -> U -> option U) :=
  forall p x y r, valid (x \+ (p \+ y)) ->
  f x (p \+ y) = Some r ->
  valid (r \+ p \+ y) /\ f (x \+ p) y = Some (r \+ p).

Lemma localV U f x y r :
  @local U f -> valid (x \+ y) -> f x y = Some r ->
  valid (r \+ y).
Proof.
by move=> H V E; move: (H Unit x y r); rewrite unitL !unitR; case. Qed.
