...
From mathcomp
Require Import ssrnat eqtype seq.
From fcsl
Require Import pred ordtype pcm unionmap.
...
Fixpoint onth (s : seq A) n : option A :=
  if s is x :: sx then if n is nx.+1 then onth sx nx else Some x
  else None.
...
by rewrite -(addn1 n) -(addn1 (size s)) ltn_add2r; apply: IH. Qed.
...
Proof.
by elim: s n => //= a s IH [[->]|n /IH]; rewrite inE ?eq_refl // orbC => ->. Qed.
...
Structure ctx := Context {
  keyx : seq K; varx : seq U}.

Definition empx := Context [::] [::].

Definition sub_ctx i j :=
...
Proof.
by case=> K1 V1 [K2 V2]; split; [move: K2 | move: V2]; apply: prefix_trans. Qed.
...
Definition interp' i t :=
  match t with Pts n v =>
    if onth (keyx i) n is Some k then pts k v else um_undef
  | Var n => if onth (varx i) n is Some f then f
    else um_undef
  end.
...
Proof.
by elim: ts x => [|t ts IH] x; rewrite /= ?unitR // IH joinCA. Qed.

Lemma interp_rev i ts : interp i (rev ts) = interp i ts.
Proof.
  elim: ts => [|t ts IH] //=; rewrite rev_cons -cats1.
by rewrite /interp foldr_cat fE /= unitR IH. Qed.
...
Proof.
by elim: ts => [|t ts /= <-] //; case: ts => //; rewrite unitR. Qed.
...
Lemma keyN i n ts :
  ~~ has (key n) ts -> interp i (kfree n ts) = interp i ts.
Proof.
  by elim: ts => [|t ts IH] //=; case: ifP => //= _ /IH->. Qed.

Lemma varN i n ts :
  ~~ has (var n) ts -> interp i (vfree n ts) = interp i ts.
Proof.
  by elim: ts => [|t ts IH] //=; case: ifP => //= _ /IH->. Qed.
...
-
  ...
  by case/(IH H)=> v ->; exists v; rewrite joinCA. Qed.

Lemma varP i n u ts : has (var n) ts -> onth (varx i) n = Some u ->
  interp i ts = u \+ interp i (vfree n ts).
Proof.
elim: ts=> [|t ts IH] //=; case: ifP=> [|_ H].
-
by case: t => //= _ /eqP <- _ ->. by move/(IH H)=> ->; rewrite joinCA. Qed.

(* interpretation is invariant under context weakening *)
(* under assumption that the interpreted term is well-formed *)

Definition wf i t :=
  match t with Pts n _ => n < size (keyx i)
  | Var n => n < size (varx i)
  end.
...
by case: t H => [n v|v] H; apply: leq_trans H _. Qed.

Lemma sc_interp i j ts :
  sub_ctx i j -> all (wf i) ts -> interp i ts = interp j ts.
Proof.
case=> H1 H2; elim: ts=> [|t ts IH] //= /andP[H] /IH->. by case: t H => [n v|n] /= /prefix_onth <-. Qed.
...
by case X: (onth _ _) => [a|]; rewrite ?(onth_size X) // valid_undef. Qed.

Lemma wf_kfree i n ts :
  all (wf i) ts ->
  all (wf i) (kfree n ts).
Proof.
by elim: ts => //= t ts IH; case: ifP => _ /andP[] //= -> /IH->. Qed.
...
Proof.
by elim: ts => //= t ts IH; case: t => [m v|m] //; rewrite inE IH. Qed.
...
Fixpoint subterm ts1 ts2 :=
  match ts1 with Pts n _ :: tsx1 => if has (key n) ts2 then subterm tsx1 (kfree n ts2) else false
  | Var n :: tsx1 =>
...
by exists xs; rewrite -joinA; apply: domeqUn D. Qed.
...
Lemma subtract_sound i ts1 ts2 rs1 rs2 xs : all (wf i) ts1 -> all (wf i) ts2 ->
  subtract ts1 ts2 xs = (rs1, rs2) ->
  exists u, dom_eq (interp i ts1 \+ interp i xs)
          (interp i rs1 \+ u) /\ dom_eq (interp i ts2) (interp i rs2 \+ u).
Proof.
...
exists (pts x v \+ u); rewrite -joinA !(pull (pts x _)). by split=> //; apply: domeqUn.
-
...
by exists (x \+ u); rewrite -joinA !(pull x); split=> //; apply: domeqUn.
  by case/(IH _ _ A1 A2)=> u [/= H1 H2]; rewrite X joinCA joinA in H1; exists u. Qed.
...
Definition isundef i ts :=
  ~~ uniq (getkeys ts) || has (undefx i) ts.
...
by case: (keyP V X) => u ->; rewrite joinA pts_undef join_undefL valid_undef. Qed.
...
Proof.
by case: f => pv [H1 H2]; split=> //; apply/prefix_cons. Qed.
...
Structure form i j ts :=
  Form {
  pivot : tagged_map;
  _ : axiom i j ts pivot}.
...
-
  ...
  by rewrite all_cat (sc_wf S2 W1) W2. Qed.
...
Lemma pts_pf vars keys1 keys2 k v (f : xfind keys1 keys2 k) :
  axiom (Context keys1 vars) (Context keys2 vars) [:: Pts k v] (key_tag (pts (xuntag f) v)).
Proof.
by case: f => p[E H]; split=> //=; rewrite ?E ?unitR // (onth_size E). Qed.
...
Lemma var_pf keys vars1 vars2 n (f : xfind vars1 vars2 n) :
  axiom (Context keys vars1) (Context keys vars2) [:: Var T n] (var_tag (xuntag f)).
Proof.
by case: f => p[E H]; split=> //=; rewrite ?E ?unitR // (onth_size E). Qed.

Canonical var_form keys vars1 vars2 v f :=
  Form (@var_pf keys vars1 vars2 v f).
...
Lemma validO j k ts1 ts2 (f1 : form (@empx K T U) j ts1) (f2 : form j k ts2) :
  valid (untag f1) -> subterm ts2 ts1 -> valid (untag f2).
Proof.
  case: f1 f2 => f1[<- _ A1] [f2] [<- S2 A2] /= V; rewrite (sc_interp S2 A1) in V.
by case/(subterm_sound A2 (sc_wf S2 A1))=> xs /domeqP[]; rewrite V => /validL->. Qed.
...
Example ex0 (x y z : nat) (v1 v2 : nat) h :
  ...
  valid (x \\-> v2 \+ Unit).
Proof. by move=> V; rewrite (validO V).
Abort.
...
case/(subterm_sound A2 (sc_wf S A1))=> xs. by case/domeqP; rewrite V => /validL->. Qed.

Canonical start j k ts1 ts2 f2 :=
  RForm (@start_pf j k ts1 ts2 f2).
...
Proof.
by case: g f1; case=> pivot H [f1] [<- Sc A] /(H A); apply. Qed.
End Exports.
...
Lemma domeqO j k rs1 rs2 ts1 ts2 (f1 : form (empx U) j ts1) (f2 : form j k ts2) :
...
rewrite domeq_sym; apply: domeq_trans D2 _. by rewrite domeq_sym; apply: domeqUn. Qed.
...
Definition raxiom j k ts1 m b (pivot : packed_map m) :=
  all (wf j) ts1 -> sub_ctx j k /\ (dom_eq (interp k b.1) (interp k b.2) ->
...
by rewrite !pp_interp interp_rev (sc_interp S A1). Qed.
End Exports.
...
Lemma undefX m i ts (g : rform i ts m true) :
  unpack (pivot g) = um_undef.
Proof.
by case: g; case=> pivot /= /(_ (erefl _))/negbT/invalidE. Qed.
