...
From fcsl
Require Import prelude ordtype pcm finmap unionmap.
...
Definition natmapMixin A := UMCMixin (@NMDef.ftE A) (@NMDef.tfE A) (@NMDef.defE A)
...
rewrite /UM.empty UMC.eqE UM.umapE /supp fmapE /= {H H1}. by elim: s. Qed.
...
by apply: contraT; rewrite negbK; apply: dom_cond. Qed.

Lemma dom_lastkey h : valid h -> ~~ empb h -> last_key h \in dom h.
Proof.
by move=> X; apply: contraR; move/(lastkey_dom X)=> ->; apply/empbP. Qed.

Lemma lastkeyxx h : valid h -> last_key h = 0 -> h = Unit.
Proof.
by move=> V H; apply: lastkey_dom V _; apply/negP=> /dom_cond; rewrite H. Qed.
...
by apply: dom_lastkey V (contraL _ H) => /empbE->; rewrite lastkey0. Qed.

Lemma lastkey_max h x : x \in dom h -> x <= last_key h.
Proof.
rewrite /last_key /= !umEX; case: (UMC.from h)=> //; case=> s H _ /=.
rewrite /supp /ord /= (leq_eqVlt x) orbC. by apply: sorted_last_key_max (sorted_oleq H). Qed.

Lemma max_lastkey h x :
  x \in dom h -> {in dom h, forall y, y <= x} -> last_key h = x.
Proof.
rewrite /last_key !umEX; case: (UMC.from h)=> //; case=> s H _ /=.
move=> H1 /= H2; apply: sorted_max_key_last (sorted_oleq H) H1 _. by move=> z /(H2 z); rewrite leq_eqVlt orbC. Qed.
...
by apply: sub_path H => z y; rewrite /oleq=> ->. Qed.
...
Proof.
move=> V; apply/leq_trans/lastkey_mono. by move=> x D; rewrite domUn inE V D. Qed.
...
by rewrite E (dom_lastkey (validR V) (negbT E2)). Qed.

Lemma lastkeyUn h1 h2 :
  last_key (h1 \+ h2) = if valid (h1 \+ h2) then maxn (last_key h1) (last_key h2) else 0.
Proof.
...
- by apply: H. by move/esym/(lastkeyUn0 V)=> E; rewrite !E unitL. Qed.
...
- by rewrite -lt0n; apply: leq_ltn_trans L.
  by apply: contraL L; move/lastkey_max; case: leqP. Qed.
...
Proof.
  by apply: contra (@dom_ordfresh _ _) _; rewrite -leqNgt leq_addr. Qed.

Lemma dom_freshUn h1 h2 :
  valid h1 ->
  [pcm h2 <= h1] ->
  fresh h1 \notin dom h2.
Proof.
move=> V[x E]; rewrite {h1}E in V *; apply: contra (@dom_ordfresh _ _) _. by rewrite joinC in V *; rewrite -leqNgt; apply: lastkeyUnf. Qed.
...
Proof.
move=> V[x E]; rewrite {h1}E in V *. by rewrite validPtUn dom_freshUn // andbT. Qed.

Lemma valid_fresh h v :
  valid (fresh h \-> v \+ h) = valid h.
Proof. by rewrite validPtUn dom_fresh andbT. Qed.

Lemma lastkey_freshUn h1 h2 v :
  valid h1 -> [pcm h2 <= h1] -> last_key (fresh h1 \-> v \+ h2) = fresh h1.
Proof.
...
rewrite leq_eqVlt; case: eqP => //= _ D.
  by apply: lastkey_max; rewrite domUn inE V D. Qed.

Lemma lastkey_fresh h v :
  valid h -> last_key (fresh h \-> v \+ h) = fresh h.
Proof.
move=> Vf; apply: max_lastkey=> [|x] /=.
-
by rewrite domUn inE valid_fresh Vf domPt inE eq_refl.
rewrite domUn inE /= valid_fresh Vf /= domPt inE /= eq_sym. by rewrite leq_eqVlt; case: eqP => //= _; apply: dom_ordfresh. Qed.
...
Proof.
by move=> V H; case: H V => z-> V; apply: lastkey_mono=> k D; rewrite domUn inE V D. Qed.

(* backward induction on valid natmaps *)

Lemma valid_indb (P : natmap A -> Prop) : P Unit -> (forall u, P (1 \-> u)) ->
    (forall t u h, P h -> last_key h < t -> valid (t \-> u \+ h) -> P (t \-> u \+ h)) ->
...
apply: H3=> //; first by apply: H (validR V).
  apply: K; apply: contraR (negbT E). by rewrite -empbE; apply: lastkey_dom (validR V).
move/empbE: {K H} E V => ->; rewrite unitR => V.
move: (H3 k v Unit H1); rewrite unitR lastkey0 lt0n.
  by apply=> //; rewrite validPt /= in V. Qed.

(* forward induction on valid natmaps *)

Lemma valid_indf (P : natmap A -> Prop) : P Unit -> (forall t u h, P h -> (forall x, x \in dom h -> t < x) ->
  valid (t \-> u \+ h) -> P (t \-> u \+ h)) ->
  forall h, valid h -> P h.
Proof.
...
-
rewrite lastkey_fresh // domPtUn inE V' /= (leq_eqVlt k) eq_sym. by move: (H k); rewrite -(ltnS k); case: ltngtP.
rewrite -(ltnS k) -/(fresh h); case/andP=> Z N.
move: (H k); rewrite lastkey_fresh // domPtUn inE V' Z /= leq_eqVlt eq_sym. by case: ltngtP N => //= _ _; apply. Qed.

Lemma gpPtUn h k u : valid (k \-> u \+ h) ->
...
-
  by rewrite domPt inE; case/andP=> _ /eqP->. by move/lastkey_max/leq_ltn_trans/(_ N)/ltnW. Qed.
...
by case E2: (find 0 h2)=> //; move/find_some/dom_cond: E2. Qed.
...
Proof.
by move=> V; rewrite /last_val /atval lastkeyPt // findPt /= V. Qed.

Lemma lastval_fresh v x h :
  valid h -> last_val v (fresh h \-> x \+ h) = x.2.
Proof.
by move=> V; rewrite /last_val /atval lastkey_fresh // findPtUn // valid_fresh. Qed.

Lemma lastvalUn v h1 h2 :
  last_val v (h1 \+ h2) =
    if valid (h1 \+ h2) then
      if last_key h1 < last_key h2 then last_val v h2 else last_val v h1 else v.
Proof.
...
by rewrite !(lastkeyUn0 V N) unitL lastkey0 find0E. Qed.

Lemma lastval_freshUn v x h1 h2 :
  valid h1 -> [pcm h2 <= h1] -> last_val v (fresh h1 \-> x \+ h2) = x.2.
Proof.
move=> V H; rewrite /last_val /atval.
  rewrite lastkey_freshUn // findPtUn // valid_freshUn //. by case: H V => h-> /validL. Qed.
...
- by move/find_some/dom_ordfresh /(ltn_trans N); rewrite ltnn. by case=> <-; rewrite N ltn_eqF.
...
by move: (C k w); rewrite !findPtUn2 // eqSS !ltn_eqF // (ltn_trans N _). Qed.

Lemma cn_sub v h x y k :
  valid (k.+1 \-> (x, y) \+ h) ->
  continuous v (k.+1 \-> (x, y) \+ h) ->
  oapp snd v (find k h) = x.
Proof.
by move=> V /(_ k (x, y)); rewrite !findPtUn2 // eq_refl ltn_eqF //; apply. Qed.
...
Proof.
by split=> //; [apply: gp0 | apply: cn0 | rewrite lastval0]. Qed.
