...
From fcsl
Require Import axioms ordtype pcm finmap unionmap.
...
Definition eq_ptr (x y : ptr) : bool :=
  match x, y with ptr_nat m, ptr_nat n => m == n
  end.

Lemma eq_ptrP : Equality.axiom eq_ptr.
Proof.
by case=> x[y] /=; case: eqP => [->|*]; constructor=> //; case. Qed.
...
by case: (ltngtP x y)=> /= E; rewrite subnKC ?(ltnW E) ?eq_refl ?orbT // E. Qed.
...
Inductive heap :=
  Undef | Def (finmap : {finMap ptr -> dynamic id}) of null \notin supp finmap.
...
Proof.
by move=> H1 H2; rewrite supp_ins negb_or /= inE /= eq_sym H1. Qed.
...
Lemma un_nullP : null \notin supp f -> null \notin supp g -> null \notin supp (fcat f g).
Proof.
by move=> H1 H2; rewrite supp_fcat negb_or H1 H2. Qed.
...
Proof.
by move=> H; apply/allP=> k; case: eqP H => //-> /negbTE->. Qed.

Lemma base_heap :
  all (fun k => k != null) (supp f) -> null \notin supp f.
Proof. by move/allP=> H; apply: (introN idP); move/H. Qed.

Lemma heapE (h1 h2 : heap) : h1 = h2 <-> match h1, h2 with Def f' pf, Def g' pg => f' = g'
      | Undef, Undef => true
      | _, _ => false
  end.
Proof.
...
rewrite {f2}E in pf1 pf2 *. by congr Def; apply: bool_irrelevance. Qed.
...
Definition dom_eq h1 h2 :=
  match h1, h2 with Def f1 _, Def f2 _ => supp f1 == supp f2
  | Undef, Undef => true
  | _, _ => false
  end.
...
Definition union h1 h2 :=
  if (h1, h2) is (Def hs1 ns1, Def hs2 ns2) then
    if disj hs1 hs2 then Def (@un_nullP _ _ ns1 ns2)
    else Undef
  else Undef.
...
Proof.
by case: f=> [|f H] //=; case: decP=> // H1; rewrite heapE. Qed.
...
by case: ifP => D//; rewrite heapE. Qed.
...
Lemma ptsE k (v : dynamic id) :
  pts k v = to (@UM.pts _ _ _ k v).
Proof.
by rewrite /pts /UM.pts /UM.upd /=; case: decP=> // H; rewrite heapE. Qed.
...
Lemma hcancelPtT2 A1 A2 x1 x2 (v1 : A1) (v2 : A2) :
  valid (x1 :-> v1) ->
  x1 :-> v1 = x2 :-> v2 -> (x1, A1) = (x2, A2).
Proof.
by move=> V; case/(cancelPt2 V)=> -> E _; rewrite E. Qed.
...
Proof.
by case/um_eta; case=> A v H; exists A, v. Qed.

Lemma heap_eta2 A x h (v : A) :
  find x h = Some (idyn v) -> h = x :-> v \+ free x h.
Proof.
move=> E; case: (heap_eta (find_some E)) => B [w] [].
rewrite {}E; case=> E; rewrite -E in w *. by move/(@inj_pair2 _ _ _ _ _)=> ->. Qed.
...
Lemma hcancel2V A x1 x2 (v1 v2 : A) h1 h2 :
  ...
  x1 :-> v1 \+ h1 = x2 :-> v2 \+ h2 ->
    ...
    else [/\ free x1 h2 = free x2 h1, h1 = x2 :-> v2 \+ free x1 h2 & h2 = x1 :-> v1 \+ free x2 h1].
...
Definition pick (h : heap) :=
  if h is Heap.Def hs _ then head null (supp hs)
  else null.
...
elim=> [|y s IH x] /=; first by case=> x; rewrite /ord /= addn1.
  by case/andP=> H1; move/IH; apply: trans H1. Qed.
...
case: ifP=> E; first by rewrite /= H1 IH.
  apply: IH; elim: s H2 => [|z s IH] //=. by case/andP=> H2 H3; rewrite (@trans _ y). Qed.
...
case/orP; first by move/eqP=> ->{z}; apply: (path_last_nat 0).
  by apply: IH; [apply: path_sorted H1 | apply: notin_path H1]. Qed.
...
Lemma emp_pick (h : heap) :
  (pick h == null) = (~~ valid h || empb h).
Proof.
rewrite /empb; case: h=> [|h] //=; case: (supp h)=> [|x xs] //=.
by rewrite inE negb_or eq_sym; case/andP; move/negbTE=> ->. Qed.

Lemma pickP h : valid h && ~~ empb h = (pick h \in dom h).
Proof.
rewrite /dom /empb; case: h=> [|h] //=. by case: (supp h)=> //*; rewrite inE eq_refl. Qed.

(***********************)
(* Some derived lemmas *)
(***********************)

Lemma domPtUnX A (v : A) x i :
  valid (x :-> v \+ i) ->
  x \in dom (x :-> v \+ i).
Proof.
  by move=> D; rewrite domPtUn inE /= D eq_refl. Qed.

Lemma domPtX A (v : A) x :
  valid (x :-> v) -> x \in dom (x :-> v).
Proof.
by move=> D; rewrite -(unitR (x :-> v)) domPtUnX // unitR. Qed.
...
Fixpoint updi x (vs : seq A) {struct vs} : heap :=
  if vs is v' :: vs' then x :-> v' \+ updi (x .+ 1) vs'
  else Unit.

Lemma updiS x v vs:
  updi x (v :: vs) = x :-> v \+ updi (x .+ 1) vs.
Proof. by []. Qed.
...
- by rewrite ptr0 unitR unitL.
  by rewrite -(addn1 (size vs)) addnC -ptrA IH joinA. Qed.
...
- by rewrite ptr0 unitL.
  by rewrite -(addn1 (size vs1)) addnC -ptrA IH joinA. Qed.

Lemma updi_catI x y vs1 vs2 :
  y = x .+(size vs1) ->
  updi x vs1 \+ updi y vs2 = updi x (vs1 ++ vs2).
...
rewrite ptrA domPtUn inE /= negb_and negb_or -{4}(ptr0 x) ptrK -lt0n H /=.
  by rewrite orbC IH // addn1. Qed.

Lemma updiD x xs :
  valid (updi x xs) = (x != null) || (size xs == 0).
Proof.
elim: xs x=> [|v xs IH] x //=; first by rewrite orbC.
by rewrite validPtUn updiVm' // orbF IH ptr_null andbF andbC. Qed.
...
case: xs=> [|v xs]; rewrite ptr0 ?andbF ?andbT //=.
  by rewrite domPtUn inE /= eq_refl -updiS updiD orbF andbT /=. Qed.
...
by rewrite -updiS updiD H /= -{1}(ptr0 x) ptrA ptrK. Qed.
...
rewrite updiVm; case/and3P=> H1; move/eqP=> -> H2.
  by split=> //; exists 0; rewrite ptrA addn0 ptr0. Qed.
...
by case: updiP => //[[H]] [m] [Ez S]; elim: X; split=> //; exists m; rewrite Ez E. Qed.
...
Structure partition (k r : heap) :=
  Partition {heap_of :> tagged_heap; _ : partition_axiom k r heap_of
}.
...
Lemma domPtUnO A h x (v : A) (f : partition (x :-> v) h) :
  dom (untag f) =i [pred y | valid (untag f) & (x == y) || (y \in dom h)].
...
Lemma updPtUnO A1 A2 x i (v1 : A1) (v2 : A2) (f : forall k, partition k i) :
...
Lemma cancelTO A1 A2 h1 h2 x (v1 : A1) (v2 : A2)
      (f1 : partition (x :-> v1) h1) (f2 : partition (x :-> v2) h2) :
...
Lemma cancelO A h1 h2 x (v1 v2 : A) (f1 : partition (x :-> v1) h1) (f2 : partition (x :-> v2) h2) :
   valid (untag f1) -> f1 = f2 :> heap -> [/\ v1 = v2, valid h1 & h1 = h2].
...
Lemma llist_prec A p (l1 l2 : seq A) h1 h2 g1 g2 :
  valid (h1 \+ g1) ->
  llist p l1 h1 -> llist p l2 h2 -> h1 \+ g1 = h2 \+ g2 -> [/\ l1 = l2, h1 = h2 & g1 = g2].
Proof.
...
by case: (IH _ _ _ _ H1 H2 E V) => -> -> ->. Qed.

Lemma first_exists A p h (ls : seq A) :
  p != null -> llist p ls h -> exists x r h',
      [/\ ls = x :: behead ls, h = p :-> (x, r) \+ h' & llist r (behead ls) h'].
Proof.
case: ls=> [|a ls] /= H []; first by case: eqP H.
by move=> r[h'] [-> H1]; eexists a, r, h'. Qed.
...
Proof.
case: xs=> [|x xs] /= V H; first by case: H. by case: H V => p[h'] [-> _] /validPtUn_cond. Qed.
From mathcomp Require Import fintype tuple finfun.
Definition indx {I : finType} (x : I) :=
  index x (enum I).
...
by rewrite -drop_nth ?index_mem ?mem_enum // cat_take_drop. Qed.
...
rewrite map_take map_drop size_takel ?joinA ?ptrA ?addn1 //. by rewrite size_map index_size. Qed.
...
move: (enum_uniq I); rewrite {1}(enum_split k) cat_uniq /= => H4.
  move=> y H5; rewrite /f' /= !ffunE /=; case: eqP H5 H4=> //-> ->. by rewrite andbF. Qed.
...
Proof.
by rewrite !fgraph_codom /= !codomE !size_map. Qed.

Lemma fgraphE T (r1 r2 : {ffun I -> T}) :
  fgraph r1 = fgraph r2 -> r1 = r2.
Proof.
by move=> eq_r12; apply/ffunP=> x; rewrite -[x]enum_rankK -!tnth_fgraph eq_r12. Qed.
