...
From mathcomp
Require Import ssrnat eqtype seq path.
From fcsl
Require Import ordtype finmap pcm.
...
Lemma all_supp_insP k v :
  p k -> all p (supp f) -> all p (supp (ins k v f)).
Proof.
move=> H1 H2; apply/allP=> x; rewrite supp_ins inE /=.
by case: eqP => [->|_] //=; move/(allP H2). Qed.

Lemma all_supp_remP k :
  all p (supp f) -> all p (supp (rem k f)).
Proof.
move=> H; apply/allP=> x; rewrite supp_rem inE /=.
by case: eqP => [->|_] //=; move/(allP H). Qed.
...
by case/orP; [move/(allP H1) | move/(allP H2)]. Qed.

Lemma all_supp_kfilterP q :
  all p (supp f) -> all p (supp (kfilter q f)).
Proof.
move=> H; apply/allP=> x; rewrite supp_kfilt mem_filter. by case/andP=> _ /(allP H). Qed.

End FormationLemmas.
Implicit Types (k : K) (v : V) (f g : base).

Lemma umapE (f g : base) : f = g <-> match f, g with Def f' pf, Def g' pg => f' = g'
  | Undef, Undef => true
  | _, _ => false
  end.
Proof.
  split; first by move=> ->; case: g.
case: f; case: g=> //f pf g pg E; rewrite {g}E in pg *. by congr Def; apply: bool_irrelevance. Qed.
...
Definition upd k v f :=
  if f is Def fs fpf then
    ...
    Def (all_supp_insP v pf fpf) else Undef
  else Undef.

Definition dom f : seq K := if f is Def fs _ then supp fs else [::].

Definition dom_eq f1 f2 :=
  match f1, f2 with Def fs1 _, Def fs2 _ => supp fs1 == supp fs2
  | Undef, Undef => true
  | _, _ => false
  end.
...
Definition union f1 f2 :=
  if (f1, f2) is (Def fs1 pf1, Def fs2 pf2) then
    if disj fs1 fs2 then Def (all_supp_fcatP pf1 pf2) else Undef
  else Undef.
...
(* forward induction *)
Lemma base_indf (P : base -> Prop) : P Undef -> P empty ->
  (forall k v f, P f -> valid (union (pts k v) f) ->
   path ord k (dom f) -> P (union (pts k v) f)) -> forall f, P f.
Proof.
...
by rewrite E fcat_inss // fcat0s. Qed.
...
Lemma base_indb (P : base -> Prop) :
  ...
  (forall x, x \in dom f -> ord x k) ->
   P (union (pts k v) f)) -> forall f, P f.
Proof.
rewrite /empty => H1 H2 H3; apply: base_ind=> //.
apply: fmap_ind''=> [|k v f S IH] H.
-
...
Structure mixin_of (T : Type) := Mixin {
  ...
  _ : forall f1 f2, union_op f1 f2 = to_op (UM.union (from_op f1) (from_op f2));
...
Proof.
by split=> [E|-> //]; rewrite -[b1]ftE -[b2]ftE E. Qed.
...
Lemma updE k v f :
  upd k v f = to (UM.upd k v (from f)).
...
Lemma unionE f1 f2 :
  union f1 f2 = to (UM.union (from f1) (from f2)).
Proof. by case: U f1 f2 => x[p] [*]. Qed.
...
by case: ifP=> E; rewrite disjC E // fcatC. Qed.
...
by case E3: (disj f1' f2')=> //; rewrite fcatAC // E1 E2 E3. Qed.
...
by case: (UMC.from f)=> [//| f' H]; rewrite disjC disj_nil fcat0s. Qed.
...
Definition union_map_classPCMMix :=
  PCMMixin (@joinC K V U)
  (@joinA K V U) (@unitL K V U) (@validL K V U) (validU U).
...
Lemma joinKf f1 f2 f :
  valid (f1 \+ f) -> f1 \+ f = f2 \+ f -> f1 = f2.
Proof.
rewrite -{3}[f1]UMC.tfE -{2}[f2]UMC.tfE !pcmE /= !umEX /UM.valid /UM.union.
case: (UMC.from f) (UMC.from f1) (UMC.from f2) => [| f' H]; case=> [|f1' H1]; case=> [|f2' H2] //; case: ifP=> //; case: ifP => //D1 D2 _ E. by apply: fcatsK E; rewrite D1 D2. Qed.
...
- by rewrite UM.umapE. by case. Qed.
...
by case: f'; case=> [|kv s] //= P /= /(_ kv.1); rewrite inE eq_refl. Qed.
...
by case: ifP => E //; rewrite supp_fcat. Qed.

Lemma dom_valid k f : k \in dom f -> valid f.
Proof.
by rewrite pcmE /= !umEX; case: (UMC.from f). Qed.

Lemma dom_cond k f : k \in dom f -> cond U k.
Proof.
by rewrite !umEX; case: (UMC.from f)=> [|f' F] // /(allP F). Qed.

Lemma dom_inIL k f1 f2 :
  valid (f1 \+ f2) -> k \in dom f1 -> k \in dom (f1 \+ f2).
Proof.
  by rewrite domUn inE => -> ->. Qed.
...
by case: (UMC.from f)=> [|f' H] //; apply: rem_supp. Qed.

CoInductive dom_find_spec k f : bool -> Type :=
  | dom_find_none' : find k f = None -> dom_find_spec k f false
  | dom_find_some' v : find k f = Some v -> f = upd k v (free k f) -> dom_find_spec k f true.
...
by case: ifP => [/eqP-> //|]; rewrite fnd_rem => ->. Qed.
...
case: (UMC.from f)=> [|f' H] x; first by rewrite !inE /= andbF. by rewrite supp_kfilt mem_filter. Qed.
...
by move=> m s S; case: disjP=> //; move/(_ _ S)/negbTE; rewrite fnd_fcat=> ->. Qed.
...
by move/negbTE: D1=> ->; move/negbTE: D2=> ->. Qed.
...
rewrite (eq_in_kfilter E1) (eq_in_kfilter E2). by rewrite kfilter_predT kfilter_pred0 fcats0. Qed.

Lemma sorted_dom f : sorted (@ord K) (dom f).
Proof.
by rewrite !umEX; case: (UMC.from f)=> [|[]]. Qed.

Lemma uniq_dom f : uniq (dom f).
Proof.
apply: sorted_uniq (sorted_dom f); by [apply: ordtype.trans | apply: ordtype.irr]. Qed.
...
case: (UMC.from f1) (UMC.from f2) Vh => // f1' H1 [//|f2' H2]. by case: disjP => //H _; apply: contraL (H x). Qed.
...
by case: (UMC.from f)=> [|f' H] //=; apply: eq_in_kfilter. Qed.

Lemma umfilt0 q : um_filter q Unit = Unit :> U.
Proof.
by rewrite !pcmE /= !umEX /UM.um_filter /UM.empty kfilt_nil. Qed.
...
rewrite !pcmE /= !umEX /UM.valid /UM.union.
  case: (UMC.from f1)=> [|f1' H1]; case: (UMC.from f2)=> [|f2' H2] //=.
by case: ifP => D//= _; rewrite kfilt_fcat disj_kfilt. Qed.

Lemma umfilt_pred0 f : valid f -> um_filter pred0 f = Unit.
Proof.
rewrite !pcmE /= !umEX /UM.valid /UM.empty.
case: (UMC.from f)=> [|f' H] //= _; case: f' H => f' P H. by rewrite fmapE /= /kfilter' filter_pred0. Qed.

Lemma umfilt_predT f : um_filter predT f = f.
Proof.
rewrite -[f]UMC.tfE !umEX /UM.um_filter.
by case: (UMC.from f)=> [|f' H] //; rewrite kfilter_predT. Qed.

Lemma umfilt_predI p1 p2 f :
  um_filter (predI p1 p2) f = um_filter p1 (um_filter p2 f).
Proof.
rewrite -[f]UMC.tfE !umEX /UM.um_filter.
by case: (UMC.from f)=> [|f' H] //; rewrite kfilter_predI. Qed.

Lemma umfilt_predU p1 p2 f :
  um_filter (predU p1 p2) f = um_filter p1 f \+ um_filter (predD p2 p1) f.
Proof.
...
by apply: eq_in_kfilter => x _; rewrite /= orb_andr orbN. Qed.
...
by move=> k _ /=; case X: (p k)=> //=; move/D/negbTE: X. Qed.
...
apply/andP/idP=> [[//] |H1]; split=> //; move: H1.
  rewrite -{1}E domUn inE /= !dom_umfilt inE /= inE /=. by case: (x \in p)=> //=; rewrite andbF.
rewrite -{2}[f1]unitR in E F; move/(dom_prec V' E): F => X.
by rewrite X in E V' *; rewrite (joinxK V' E). Qed.
...
-
by rewrite H2 !umEX kfilt_ins H1. by case: ifP H2. Qed.

Lemma umfiltF p k f :
  um_filter p (free k f) = if p k then free k (um_filter p f) else um_filter p f.
Proof.
rewrite !umEX /UM.um_filter /UM.free.
by case: (UMC.from f)=> [|f' F]; case: ifP=> //E; rewrite !umEX kfilt_rem E. Qed.
...
Proof.
by rewrite !pcmE /= !umEX -2![f]UMC.tfE !umEX; case: (UMC.from f). Qed.

Lemma valid_undef : valid (um_undef : U) = false.
Proof. by apply/negbTE; apply/invalidE. Qed.

Lemma validU k v f :
  valid (upd k v f) = cond U k && valid f.
Proof.
rewrite !pcmE /= !umEX /UM.valid /UM.upd /cond.
case: (UMC.from f)=> [|f' F]; first by rewrite andbF. by case: decP=> [|/(introF idP)] ->. Qed.
...
| valid_false2 of ~~ valid f2 : validUn_spec f1 f2 false
  ...
  | valid_true of valid f1 & valid f2 & (forall x, x \in dom f1 -> x \notin dom f2) : validUn_spec f1 f2 true.
...
by apply: (valid_false3 (k := k)); rewrite !umEX. Qed.
...
-
by move=> k'; rewrite domF inE; case: eqP => // _ /H/negbTE->. by rewrite validF V1. by rewrite V2. Qed.
...
-
  move=> V' _; apply/esym/negbTE; apply: contra V'; move/validR. by rewrite validU; case/andP.
-
...
by rewrite (negbTE H3); case: ifP H2 H3 => ///eqP-> ->. Qed.
...
- by constructor; case.
- by constructor; case.
by case: eqP => H; constructor; [rewrite H | case=> _ /suppE]. Qed.

Lemma domeq0E f : dom_eq f Unit -> f = Unit.
Proof.
  by case/domeqP; rewrite valid_unit dom0; apply: dom0E. Qed.
...
- by rewrite -E Vg1.
- by rewrite Vf. by move=> x; rewrite -F; move/H/negbTE=> ->.
...
by rewrite !(joinC f1); move/(L _ _ _ H'). Qed.
...
by rewrite (domeqVUnE F (domeq_refl g)) H. Qed.
...
by rewrite -joinA joinCA joinA; apply: domeqUn. Qed.

Lemma domeqUnf f f1 f2 f1' f2' :
  dom_eq f1 (f \+ f2) -> dom_eq (f \+ f1') f2' ->
  dom_eq (f1 \+ f1') (f2 \+ f2').
Proof. by move=> D1 D2; rewrite (joinC f1) (joinC f2); apply: domeqfUn D2 D1. Qed.
...
-
  ...
  by move=> k /Hs; rewrite E in V1; case: validUn V1 => // _ _ L _ /L/ negbTE->. Qed.

Lemma prec_filtV x1 x2 y1 y2 :
  valid (x1 \+ y1) -> x1 \+ y1 = x2 \+ y2 ->
  reflect (x1 = um_filter (mem (dom x1)) x2)
          (valid (x1 \+ y2)).
Proof.
move=> V1 E; case X: (valid (x1 \+ y2)); constructor; last first.
-
case: (prec_domV V1 E) X => //St _ H; apply: St. by move=> n; rewrite H dom_umfilt inE; case/andP.
...
by move=> n; case: validUn X => // _ _ L _ /(contraL (L _))/negbTE. Qed.
...
by rewrite !fnd_ins eq_refl; case. Qed.
...
-
by rewrite (eqP E) in H2 *. by rewrite ins_ins E. Qed.
...
by case: decP => H2//; rewrite !umEX ins_rem H1. Qed.
...
by rewrite disj_ins D andbF. Qed.
...
Proof.
by rewrite !pcmE /= !umEX /UM.free /UM.empty rem_empty. Qed.
...
- by case: ifP H2. by rewrite !umEX rem_ins H1. Qed.

Lemma freeF k1 k2 f :
  free k1 (free k2 f) = if k1 == k2 then free k1 f else free k2 (free k1 f).
Proof.
rewrite !umEX /UM.free.
by case: (UMC.from f)=> [|f' F]; case: ifP=> //E; rewrite !umEX rem_rem E. Qed.

Lemma freeUn k f1 f2 : free k (f1 \+ f2) =
    if k \in dom (f1 \+ f2) then free k f1 \+ free k f2 else f1 \+ f2.
Proof.
...
by case: disjP E1 E2 => //H _; move/contra: (H k); rewrite negbK. Qed.

Lemma freeUnV k f1 f2 :
  valid (f1 \+ f2) -> free k (f1 \+ f2) = free k f1 \+ free k f2.
Proof.
move=> V'; rewrite freeUn domUn V' /=; case: ifP=> //.
by move/negbT; rewrite negb_or; case/andP=> H1 H2; rewrite !dom_free. Qed.
...
by case: ifP H3 V1 => [|_ _ _]; [move/eqP=> <- -> | move/negbTE=> ->]. Qed.
...
- by rewrite kfilt_rem E. by rewrite rem_kfilt E. Qed.
...
Proof.
by rewrite pcmE /= !umEX /UM.find /= fnd_empty. Qed.

Lemma find_undef k : find k (um_undef : U) = None.
Proof.
by rewrite !umEX /UM.find. Qed.
...
by apply: contra H; case: allP F => //F _ /F. Qed.
...
by rewrite (introF idP H) /= if_same. Qed.

Lemma findF k1 k2 f :
  find k1 (free k2 f) = if k1 == k2 then None else find k1 f.
Proof.
rewrite !umEX /UM.find /UM.free.
case: (UMC.from f)=> [|f' F]; first by rewrite if_same. by rewrite fnd_rem. Qed.
...
by rewrite fnd_supp ?E1 // fnd_supp ?E2.
  suff E2: k \notin supp f2' by rewrite fnd_fcat (negbTE E2). by case: disjP D E1 => //H _; apply: H. Qed.
...
by rewrite !umEX; apply/fmapP=> k; move: (H k); rewrite !umEX. Qed.

Lemma find_umfilt p k f : find k (um_filter p f) = if p k then find k f else None.
Proof.
rewrite !umEX /UM.find /UM.um_filter.
case: (UMC.from f)=> [|f' F]; first by rewrite if_same. by rewrite fnd_kfilt. Qed.
...
- by move/supp_nilE: E => ->. by move=> H; rewrite H in E. Qed.

Lemma empbU k v f : empb (upd k v f) = false.
Proof.
rewrite !umEX /UM.empb /UM.upd.
case: (UMC.from f)=> [|f' F] //; case: decP=> //H.
suff: k \in supp (ins k v f') by case: (supp _). by rewrite supp_ins inE /= eq_refl. Qed.
...
by exists x; rewrite inE eq_refl.
  suff: k \in supp (fcat f1' f2') by rewrite E1. by rewrite supp_fcat inE /= H3. Qed.
...
apply/supp_nilE; case: (supp f') H => // x s /(_ x). by rewrite inE eq_refl. Qed.
...
by case E: (k \in dom f)=> //; move: (L k E); rewrite E. Qed.

Lemma empb_umfilt p f : empb f -> empb (um_filter p f).
Proof.
rewrite !umEX /UM.empb /UM.um_filter.
case: (UMC.from f)=> [|f' F] //. by move/eqP/supp_nilE=> ->; rewrite kfilt_nil. Qed.
...
by case: (UMC.from f)=> [|f' F]; constructor; rewrite !umEX. Qed.
...
by move=> X Y k v; rewrite findUnL //; case: ifP=> _; [apply: X|apply: Y]. Qed.

Lemma umallUnL f1 f2 :
  valid (f1 \+ f2) -> um_all (f1 \+ f2) -> um_all f1.
Proof.
by move=> W H k v F; apply: (H k v); rewrite findUnL // (find_some F). Qed.

Lemma umallUnR f1 f2 :
  valid (f1 \+ f2) -> um_all (f1 \+ f2) -> um_all f2.
...
Proof.
  split; last by move=> <- a; rewrite dom_umfilt; case/andP. by move/eq_in_umfilt=> ->; rewrite umfilt_predT. Qed.
...
by move=> D; move: (H a); rewrite inE /= D; move/negbT. Qed.

Lemma subset_disjC p q : {subset p <= predC q} <-> {subset q <= predC p}.
Proof. by split=> H a; apply: contraL (H a). Qed.

Lemma valid_subdom x y :
  valid (x \+ y) -> {subset dom x <= [predC dom y]}.
Proof. by case: validUn. Qed.

Lemma subdom_valid x y :
  {subset dom x <= [predC dom y]} -> valid x -> valid y -> valid (x \+ y).
Proof.
by move=> H X Y; case: validUn; rewrite ?X ?Y => // k /H/negbTE/= ->. Qed.
...
move=> k X; case: dom_find X H => // a _ -> _; rewrite umfiltU !inE.
  by case: ifP => // _ /empbE; rewrite /= empbU. Qed.
...
Proof.
  by exists um_undef; rewrite join_undefR. Qed.
...
by case/(joinxK V)/esym/join0E=> ->; rewrite unitR. Qed.
...
by case: H V => a -> V; rewrite umfiltUn //; eexists _.
  by move/invalidE: (negbT V)=> ->; rewrite umfilt_undef; apply: umpleq_undef. Qed.

(* filter p x is lower bound for x *)
Lemma umpleq_filtI x p : [pcm um_filter p x <= x].
Proof.
exists (um_filter (predD predT p) x); rewrite -umfilt_predU. by rewrite -{1}[x]umfilt_predT; apply: eq_in_umfilt=> a; rewrite /= orbT. Qed.
...
Proof.
by move=> V; split=> [|<-] // H; case: H V => b->; apply: umfilt_dom. Qed.
...
move=> V /(umpleq_filtE _ Vx) <- /(umpleq_filtE _ Vx) <-. by rewrite -umfilt_dpredU //; apply: valid_subdom V. Qed.

(* x <= y and subdom *)

Lemma umpleq_subdom x y :
  valid y -> [pcm x <= y] -> {subset dom x <= dom y}.
Proof.
  by move=> V H; case: H V => a -> V b D; rewrite domUn inE V D. Qed.
...
rewrite umfilt_memdomE; move/subset_disjC: (valid_subdom V). by move/(subdom_umfilt0 _ (validR V))=> ->; rewrite unitR. Qed.
...
by case: validUn V => // _ _ L _ /L/negbTE->. Qed.
...
Proof.
by move=> V H; case: H V => a -> V H; rewrite findUnL // (find_some H). Qed.

Lemma umpleq_none x1 x2 t :
  valid x2 -> [pcm x1 <= x2] -> find t x2 = None -> find t x1 = None.
Proof.
by case E: (find t x1)=> [a|] // V H <-; rewrite (umpleq_some V H E). Qed.
...
Proof.
by rewrite !pcmE /= !umEX /UM.pts /UM.upd; case: decP. Qed.

Lemma domPtK k v :
  dom (pts k v : U) = if cond U k then [:: k] else [::].
Proof.
rewrite !umEX /= /UM.dom /supp /UM.pts /UM.upd /UM.empty /=.
by case D: (decP _)=> [a|a] /=; rewrite ?a ?(introF idP a). Qed.

(* a weaker version of gen_domPtK, but legacy *)
Lemma domPt k v :
  dom (pts k v : U) =i [pred x | cond U k & k == x].
Proof.
by move=> x; rewrite ptsU domU !inE eq_sym valid_unit dom0; case: eqP. Qed.

Lemma validPt k v : valid (pts k v : U) = cond U k.
Proof. by rewrite ptsU validU valid_unit andbT. Qed.

Lemma domeqPt k v1 v2 :
  dom_eq (pts k v1 : U) (pts k v2).
Proof.
by apply/domeqP; rewrite !validPt; split=> // x; rewrite !domPt. Qed.

Lemma findPt k v :
  find k (pts k v : U) = if cond U k then Some v else None.
Proof.
by rewrite ptsU findU eq_refl /= valid_unit andbT. Qed.

Lemma findPt2 k1 k2 v :
  find k1 (pts k2 v : U) =
    if (k1 == k2) && cond U k2 then Some v else None.
Proof.
  by rewrite ptsU findU valid_unit andbT find0E; case: ifP=> //=; case: ifP. Qed.

Lemma findPt_inv k1 k2 v w :
  find k1 (pts k2 v : U) = Some w ->
  [/\ k1 = k2, v = w & cond U k2].
Proof.
rewrite ptsU findU; case: eqP; last by case: ifP => //; rewrite find0E.
  by move=> ->; rewrite valid_unit andbT; case: ifP => // _ [->]. Qed.

Lemma freePt2 k1 k2 v : cond U k2 -> free k1 (pts k2 v : U) = if k1 == k2 then Unit
  else pts k2 v.
...
by case=> E'; rewrite -{k2}E' in E *; rewrite (cancelPt H E). Qed.

Lemma updPt k v1 v2 :
  upd k v1 (pts k v2) = pts k v1 :> U.
Proof.
by rewrite !ptsU updU eq_refl. Qed.
...
-
rewrite validPt=> H1 -> H2; rewrite H1 (H2 k) //. by rewrite domPtK H1 inE.
- by rewrite validPt; move/negbTE=> ->.
- by move/negbTE=> ->; rewrite andbF.
rewrite domPtK=> x; case: ifP => // _.
  by rewrite inE => /eqP-> ->; rewrite andbF. Qed.
...
Lemma validUnPt_cond k v f :
  valid (f \+ pts k v) -> cond U k.
...
Proof.
move=> x; rewrite domUn !inE !validPtUn domPt !inE. by rewrite -!andbA; case: (UMC.p _ k). Qed.

Lemma domUnPt k v f : dom (f \+ pts k v) =i
  [pred x | valid (f \+ pts k v) & (k == x) || (x \in dom f)].
Proof. by rewrite joinC; apply: domPtUn. Qed.

Lemma domPtUnE k v f :
  k \in dom (pts k v \+ f) = valid (pts k v \+ f).
Proof. by rewrite domPtUn inE eq_refl andbT. Qed.

Lemma domUnPtE k v f :
  k \in dom (f \+ pts k v) = valid (f \+ pts k v).
Proof. by rewrite joinC; apply: domPtUnE. Qed.

Lemma validxx f : valid (f \+ f) -> dom f =i pred0.
Proof.
  by case: validUn => // _ _ L _ z; case: (_ \in _) (L z)=> //; elim. Qed.
...
by rewrite ptsU findU eq_refl valid_unit (validPtUn_cond V'). Qed.
...
Lemma findPtUn2 k1 k2 v f :
  valid (pts k2 v \+ f) ->
  find k1 (pts k2 v \+ f) = if k1 == k2 then Some v else find k1 f.
Proof.
move=> V'; rewrite findUnL // domPt inE eq_sym.
  by rewrite findPt2 (validPtUn_cond V') andbT /=; case: ifP => //->. Qed.

Lemma findUnPt2 k1 k2 v f : valid (f \+ pts k2 v) ->
  find k1 (f \+ pts k2 v) = if k1 == k2 then Some v else find k1 f.
...
Lemma freePtUn2 k1 k2 v f :
  valid (pts k2 v \+ f) ->
  free k1 (pts k2 v \+ f) = if k1 == k2 then f else pts k2 v \+ free k1 f.
Proof.
...
by case: ifP => //N; rewrite dom_free // N. Qed.

Lemma freeUnPt2 k1 k2 v f :
  valid (f \+ pts k2 v) ->
  free k1 (f \+ pts k2 v) = if k1 == k2 then f else free k1 f \+ pts k2 v.
Proof.
by rewrite !(joinC _ (pts k2 v)); apply: freePtUn2. Qed.
...
-
  ...
  move/negbT/invalidE: V1 => ->; move/negbT/invalidE: V2=> ->. by apply: upd_invalid. Qed.

Lemma updUnPt k v1 v2 f :
  upd k v1 (f \+ pts k v2) = f \+ pts k v1.
...
Proof.
apply/invalidE; rewrite validPtUn validPt domPt !negb_and negb_or eq_refl. by case: (cond U k). Qed.
...
Lemma umallPtE (P : V -> Prop) k v :
  cond U k ->
  um_all P (pts k v : U) -> P v.
...
by split=> //; apply: umallPtE H1; apply: validPtUn_cond W. Qed.

(* others *)

Lemma um_eta k f :
  k \in dom f ->
  exists v, find k f = Some v /\ f = pts k v \+ free k f.
Proof.
case: dom_find => //v E1 E2 _; exists v; split=> //.
by rewrite {1}E2 -{1}[free k f]unitL updUnR domF inE /= eq_refl ptsU. Qed.

Lemma um_eta2 k v f : find k f = Some v -> f = pts k v \+ free k f.
Proof.
by move=> E; case/um_eta: (find_some E)=> v' []; rewrite E; case=> <-. Qed.

Lemma cancel k v1 v2 f1 f2 : valid (pts k v1 \+ f1) ->
...
by rewrite -{}X in E *; rewrite -(joinxK V' E) (validR V'). Qed.

Lemma cancel2 k1 k2 f1 f2 v1 v2 :
  valid (pts k1 v1 \+ f1) -> pts k1 v1 \+ f1 = pts k2 v2 \+ f2 ->
  if k1 == k2 then v1 = v2 /\ f1 = f2 else [/\ free k1 f2 = free k2 f1, f1 = pts k2 v2 \+ free k1 f2 & f2 = pts k1 v1 \+ free k2 f1].
Proof.
...
by rewrite {1}E' freePtUn // -E'; apply: (validR V2). Qed.
...
Lemma um_indf (P : U -> Prop) :
   P um_undef -> P Unit ->
   (forall k v f, P f -> valid (pts k v \+ f) -> path ord k (dom f) -> P (pts k v \+ f)) -> forall f, P f.
Proof.
rewrite !pcmE /= !umEX => H1 H2 H3 f; rewrite -[f]UMC.tfE.
apply: (UM.base_indf (P := (fun b => P (UMC.to b))))=> //.
move=> k v g H V1; move: (H3 k v _ H); rewrite !pcmE /= !umEX. by apply. Qed.

(* backward progressing over keys *)
Lemma um_indb (P : U -> Prop) : P um_undef -> P Unit ->
  (forall k v f, P f -> valid (pts k v \+ f) ->
  (forall x, x \in dom f -> ord x k) -> P (pts k v \+ f)) -> forall f, P f.
Proof.
rewrite !pcmE /= !umEX => H1 H2 H3 f; rewrite -[f]UMC.tfE.
...
Lemma um_valid3 f1 f2 f3 :
  valid (f1 \+ f2 \+ f3) = [&& valid (f1 \+ f2), valid (f2 \+ f3) & valid (f1 \+ f3)].
Proof.
apply/idP/idP.
-
...
by case: validUn V2 => // _ _ X _ /X/negbTE->. Qed.

(* points-to is a prime element of the monoid *)
Lemma um_prime f1 f2 k v : cond U k -> f1 \+ f2 = pts k v ->
  f1 = pts k v /\ f2 = Unit \/ f1 = Unit /\ f2 = pts k v.
Proof.
...
-
by move/eqP=> -> [->] /join0E[-> ->]; rewrite unitR; left. by move=> _ [_ _]/esym/empbP; rewrite empbPtUn. Qed.
...
Definition union_map_classTPCMMix K V (U : union_map_class K V) :=
  TPCMMixin (@empbP K V U) (@join0E K V U) (@valid_undefN K V U) (@undef_join _ _ U).
...
Proof.
rewrite {1}/eq_op /= /UnionMapEq.unionmap_eq /um_pts !umEX /=. by rewrite {1}/eq_op /= /feq eqseq_cons andbT. Qed.

Lemma umPt0E (k : K) (v : V) : (k \\-> v == Unit) = false.
Proof.
by apply: (introF idP)=> /eqP/empbP; rewrite empbPt. Qed.

Lemma um0PtE (k : K) (v : V) :
  (@Unit [pcm of union_map K V] == k \\-> v) = false.
Proof. by rewrite eq_sym umPt0E. Qed.

Lemma umPtUndefE (k : K) (v : V) :
  (k \\-> v == um_undef) = false.
...
Proof.
by apply/(introF idP)=> /eqP/empbP; rewrite empb_undef. Qed.
...
Proof.
by apply: (introF idP) => /eqP/join0E/proj1/eqP; rewrite umPt0E. Qed.
...
-
move/(cancelPt2 _); rewrite validPt=> /(_ (erefl _)). by case=> -> -> ->; rewrite !eq_refl empb0. by move/empbP; rewrite empbPt. Qed.

Lemma umPtPtUE (k1 k2 : K) (v1 v2 : V) f :
  (k1 \\-> v1 == k2 \\-> v2 \+ f) = [&& k1 == k2, v1 == v2 & empb f].
Proof.
by rewrite eq_sym umPtUPtE (eq_sym k1) (eq_sym v1). Qed.
...
Definition umE := (umPtPtE, umPt0E, um0PtE, umPtUndefE,
  umUndefPtE, um0UndefE, umUndef0E,
   ...
   unitL, unitR, validPt,
   valid_unit,
   eq_refl, empb0, empbPt, join_undefL, join_undefR, empb_undef).
