...
by rewrite H in H1 H2 *; rewrite (bool_irrelevance H1 H2). Qed.

Lemma sorted_nil : sorted ord (map key [::]).
Proof. by []. Qed.
Definition nil := FinMap sorted_nil.

Definition fnd k (s : fmap) :=
  if (filter (predk k) (seq_of s)) is (_, v) :: _ then Some v else None.

Fixpoint ins' (k : K) (v : V)
  (s : seq (K * V)) {struct s} :
  seq (K * V) :=
  if s is (k1, v1) :: s1 then
      if ord k k1 then (k, v) :: s else
    if k == k1 then (k, v) :: s1 else (k1, v1) :: (ins' k v s1) else [:: (k, v)].
...
case/andP=> H2 H3; case: ifP => /= H4; first by rewrite H1 H3 H4.
  ...
  by rewrite H2 IH //; move: (total k2 k'); rewrite H4 H5. Qed.

Lemma sorted_ins' s k v :
  sorted ord (map key s) ->
  sorted ord (map key (ins' k v s)).
Proof.
case: s => //[[k' v']] s /= H.
case: ifP => //= H1; first by rewrite H H1.
case: ifP => //= H2; first by rewrite (eqP H2).
  by rewrite path_ins' //; move: (total k k'); rewrite H1 H2. Qed.

Definition ins k v s := let: FinMap s' p' := s in FinMap (sorted_ins' k v p').

Lemma sorted_filter' k s :
  sorted ord (map key s) ->
  sorted ord (map key (filter (predCk k) s)).
Proof.
by move=> H; rewrite -filter_map sorted_filter //; apply: trans. Qed.
...
Lemma ord_path (x y : K) s :
  ord x y -> path ord y s -> path ord x s.
Proof.
elim: s x y => [|k s IH] x y //=.
by move=> H1; case/andP=> H2->; rewrite (trans H1 H2). Qed.
...
Lemma first_ins' (k : K) (v : V) s :
  (forall x, x \in map key s -> ord x k) -> ins' k v s = rcons s (k, v).
Proof.
elim: s=> [|[k1 v1] s IH] H //=.
move: (H k1); rewrite inE eq_refl; move/(_ (erefl _)).
  case: totalP => //O _; rewrite IH //.
by move=> x H'; apply: H; rewrite inE /= H' orbT. Qed.
...
rewrite inE negb_or; case/andP=> H1 H2.
  by rewrite eq_sym (negbTE H1); apply: IH. Qed.

Lemma fmapP (s1 s2 : fmap) :
  (forall k, fnd k s1 = fnd k s2) -> s1 = s2.
Proof.
...
- by apply: notin_path; apply: ord_path P2.
  move: (H k2); rewrite E eq_refl notin_filter //. by apply: notin_path; apply: ord_path P1. Qed.

Lemma predkN (k1 k2 : K) :
  predI (predk V k1) (predCk V k2) =1
  if k1 == k2 then pred0 else predk V k1.
Proof.
by move=> x; case: ifP => H /=; [|case: eqP =>//->]; rewrite ?(eqP H) ?andbN ?H. Qed.
...
rewrite -topredE /= eq_sym H; apply: IH. by apply: path_sorted H1.
case: s; elim=> [|[y w] s IH] /= H1 //; rewrite /supp /= inE in IH *.
case: eqP => [-> _|H] //=.
-
by apply: (@supp_spec_some _ _ w); rewrite /fnd /= eq_refl.
move: (path_sorted H1)=> H1'; move/(IH H1'); case=> [v H2|H2]; [ apply: (@supp_spec_some _ _ v)
| apply: supp_spec_none];
  by rewrite /fnd /=; case: eqP H => //->. Qed.

Lemma suppE (s1 s2 : fmap) :
  supp s1 =i supp s2 <-> supp s1 = supp s2.
Proof.
  split; last by move=> ->.
...
case: ifP => H1 /=; first by rewrite inE.
  ...
  by rewrite !inE (IH _ _ _ (path_sorted H)) orbCA. Qed.

Lemma fnd_empty k : fnd k nil = None.
Proof. by []. Qed.
...
by case: eqP => //; rewrite filter_pred0. Qed.
...
by apply: IH; apply: path_sorted H. Qed.

Lemma ins_rem k1 k2 v (s : fmap) :
  ins k1 v (rem k2 s) =
  if k1 == k2 then ins k1 v s else rem k2 (ins k1 v s).
Proof. move: k1 k2 v s.
have L3: forall (x : K) s, path ord x (map key s) -> filter (predCk V x) s = s.
-
...
case H2: (k' == x) => /=.
-
rewrite (eqP H2) eq_refl in H *. by rewrite L3 //; apply: last_ins' H.
  rewrite eq_sym H2 H1 IH //. by apply: path_sorted H.
move=> k1 k2 v [s H].
...
case H1: (k' == k2)=> /=.
-
rewrite eq_sym H1 (eqP H1) irr in H *. by rewrite L3 // last_ins'.
rewrite eq_sym H1; case: ifP => H3.
-
by rewrite L3 //; apply: ord_path H3 H. by rewrite L5 //; apply: path_sorted H.
...
by rewrite H2 IH //; apply: path_sorted H. Qed.

Lemma ins_ins k1 k2 v1 v2 (s : fmap) :
  ins k1 v1 (ins k2 v2 s) =
  if k1 == k2 then ins k1 v1 s
...
-
  ...
  case: (totalP k2 k3) H1 => H2 _ //. by rewrite IH //; apply: path_sorted H.
...
rewrite eq_sym H1. by case: (totalP k2 k1) H1 H2.
...
-
  by rewrite (eqP H4) irr in H2. by rewrite IH //; apply: path_sorted H. Qed.
...
-
  by rewrite (eqP H1) andbb. by rewrite andbC. Qed.
...
case: (totalP k3 k2) => H1 /=; rewrite ?eq_refl //=; case: (totalP k3 k2) H1 => //= H1 _. by rewrite IH //; apply: path_sorted H.
...
rewrite eq_sym H1 /=.
  ...
  rewrite last_ins' /= 1?eq_sym ?H1 //. by apply: ord_path H.
-
by move: H1; rewrite (eqP H2) /= eq_sym => -> /=; rewrite irr eq_refl.
case: (totalP k3 k1) => H3 /=.
-
  case: (totalP k2 k3) H2 => //= H2 _. by rewrite IH //; apply: path_sorted H.
-
  rewrite -(eqP H3) in H1 *. by rewrite IH //; apply: path_sorted H.
  case: (totalP k2 k3) H2 => //= H2 _. by rewrite IH //; apply: path_sorted H. Qed.

Lemma rem_supp k (s : fmap) : k \notin supp s -> rem k s = s.
Proof.
...
by rewrite eq_sym H2 H3. Qed.

Lemma fnd_supp k (s : fmap) : k \notin supp s -> fnd k s = None.
Proof. by case: suppP. Qed.

Lemma fnd_supp_in k (s : fmap) : k \in supp s -> exists v, fnd k s = Some v.
Proof. by case: suppP=> [v|]; [exists v|]. Qed.

Lemma cancel_ins k v (s1 s2 : fmap) :
   k \notin (supp s1) -> k \notin (supp s2) -> ins k v s1 = ins k v s2 -> s1 = s2.
Proof.
...
by case=> -> -> H; congr (_ :: _); apply: IH1 H. Qed.
...
Lemma seqof_ins k v (s : fmap) :
  path ord k (supp s) ->
  seq_of (ins k v s) = (k, v) :: seq_of s.
Proof.
by case: s; elim=> [|[k1 v1] s IH] //= _; case/andP=> ->. Qed.
...
by move: (IH H6 k2 k v H H4); case: s {IH p H4 H5} H6. Qed.

Lemma path_supp_ins_inv k1 k v (s : fmap) : path ord k (supp s) -> path ord k1 (supp (ins k v s)) ->
  ord k1 k && path ord k1 (supp s).
Proof.
case: s => s p; rewrite /supp /= => H1; rewrite last_ins' //=.
by case/andP=> H2 H3; rewrite H2; apply: ord_path H3. Qed.
...
have S: sorted ord (map key s) by apply: path_sorted H.
  rewrite (_ : FinMap _ = ins k v (FinMap S)); last by rewrite fmapE /= last_ins'. by apply: H2. Qed.

(* backward induction principle *)
Lemma fmap_ind'' (P : fmap -> Prop) :
    P nil -> (forall k v s, (forall x, x \in supp s -> ord x k) ->
  P s -> P (ins k v s)) -> forall s, P s.
Proof.
...
-
  by apply: H2 (IH _) => x/T. by rewrite fmapE /= first_ins'. Qed.
...
rewrite inE negb_or; case/andP=> H1 H2.
  by rewrite -IH // ins_ins eq_sym (negbTE H1). Qed.

Lemma fcat_nil' s : fcat' nil (seq_of s) = s.
Proof.
elim/fmap_ind': s => [|k v s L IH] //=.
  by rewrite seqof_ins //= (_ : FinMap _ = ins k v nil) // fcat_ins' ?notin_path // IH. Qed.

Lemma fcat0s s : fcat nil s = s.
Proof. by apply: fcat_nil'. Qed.

Lemma fcats0 s : fcat s nil = s.
Proof. by []. Qed.
...
by rewrite rem_ins E IH.
  by rewrite rem_ins E -IH // -fcat_sins. Qed.

Lemma fnd_fcat k s1 s2 :
  fnd k (fcat s1 s2) = if k \in supp s2 then fnd k s2 else fnd k s1.
Proof.
elim/fmap_ind': s2 k s1 => [|k2 v2 s2 H IH] k1 s1.
- by rewrite fcats0.
...
move=> H1; rewrite fcat_sins !fnd_ins.
  by case: ifP => //; rewrite IH H1. Qed.

Lemma supp_fcat s1 s2 :
  supp (fcat s1 s2) =i [predU supp s1 & supp s2].
Proof.
...
by rewrite supp_ins inE /= E orbT.
  by rewrite IH supp_ins inE /= inE /= E. Qed.
...
by rewrite (negbTE (notin_path H1)) (negbTE (notin_path H2)). Qed.

Lemma fmap_ind2 (P : fmap -> fmap -> Prop) : P nil nil -> (forall k v1 v2 s1 s2, path ord k (supp s1) -> path ord k (supp s2) ->
  P s1 s2 -> P (ins k v1 s1) (ins k v2 s2)) ->
  forall s1 s2, supp s1 =i supp s2 -> P s1 s2.
Proof.
move=> H1 H2; elim/fmap_ind'=> [|k1 v1 s1 T1 IH1]; elim/fmap_ind'=> [|k2 v2 s2 T2 _] //.
-
  by move/(_ k2); rewrite supp_ins inE /= eq_refl supp_nil.
- by move/(_ k1); rewrite supp_ins inE /= eq_refl supp_nil.
by case/supp_eq_ins=> // E; rewrite -{k2}E in T2 *; move/IH1; apply: H2. Qed.
...
Proof.
by case: s => s H; rewrite -filter_map path.sorted_filter //; apply: trans. Qed.
...
by rewrite -E3 E1 in E2. Qed.

Lemma kfilt_ins (p : pred K) k v (s : fmap) :
  kfilter p (ins k v s) = if p k then ins k v (kfilter p s) else kfilter p s.
Proof.
apply/fmapP=> k2; case: ifP=> E1.
-
by rewrite fnd_ins !fnd_kfilt fnd_ins; case: eqP => //->; rewrite E1.
by rewrite !fnd_kfilt fnd_ins; case: eqP => //->; rewrite E1. Qed.

Lemma rem_kfilt (p : pred K) k (s : fmap) :
  rem k (kfilter p s) = if p k then kfilter p (rem k s) else kfilter p s.
Proof.
apply/fmapP=> k2; case: ifP=> E1.
-
by rewrite fnd_rem !fnd_kfilt fnd_rem; case: eqP => //->; rewrite E1.
by rewrite fnd_rem fnd_kfilt; case: eqP => //->; rewrite E1. Qed.

Lemma kfilt_rem (p : pred K) k (s : fmap) :
  kfilter p (rem k s) = if p k then rem k (kfilter p s) else kfilter p s.
Proof.
apply/fmapP=> k2; case: ifP=> E1.
-
by rewrite fnd_kfilt !fnd_rem fnd_kfilt; case: eqP => //->; rewrite E1.
by rewrite !fnd_kfilt fnd_rem; case: eqP => //->; rewrite E1. Qed.
...
Proof.
apply/fmapP=> k; rewrite fnd_kfilt !fnd_fcat !fnd_kfilt supp_kfilt mem_filter. by case: ifP. Qed.
...
Proof.
  by apply/fmapE; rewrite /= /kfilter' filter_predI. Qed.
...
rewrite inE /=; case: (ifP (p1 k)); case: (ifP (p2 k))=> //=; by [case: ifP | case: suppP]. Qed.
...
by rewrite H // => x E; apply: H1; rewrite inE /= E orbT. Qed.
...
CoInductive disj_spec (s1 s2 : fmap) : bool -> Type :=
  ...
  | disj_false x of x \in supp s1 & x \in supp s2 : disj_spec s1 s2 false.
...
move/(nth_find k); move: H; rewrite has_find => /(mem_nth k). by apply: disj_false. Qed.

Lemma disjC s1 s2 : disj s1 s2 = disj s2 s1.
Proof.
case: disjP; case: disjP=> //.
- by move=> x H1 H2; move/(_ x H2); rewrite H1. by move=> H1 x H2; move/H1; rewrite H2. Qed.
...
by move: (H _ H1); rewrite H2. Qed.

Lemma disj_rem k (s1 s2 : fmap) : disj s1 s2 -> disj s1 (rem k s2).
Proof.
case: disjP=> //H _; case: disjP=> //x; move/H.
by rewrite supp_rem inE /= andbC; move/negbTE=> ->. Qed.
...
move=> x H1 H2 H3; move: (H3 x H1) H.
  rewrite supp_rem inE /= negb_and H2 orbF negbK. by move/eqP=> <-; rewrite H1. Qed.
...
case: (k \in supp s1)=> //=.
case: (k \in supp s2)=> //=; first by rewrite andbF. by rewrite -!(disjC s) IH. Qed.
...
by rewrite fcat_ins' // -IH // seqof_ins //= -fcat_ins' ?notin_path. Qed.
...
by rewrite fcat_sins ?notin_path // IH // fcat_sins ?notin_path // fcat_sins. Qed.

Lemma fcatAC (s1 s2 s3 : fmap) :
  [&& disj s1 s2, disj s2 s3 & disj s1 s3] ->
  fcat s1 (fcat s2 s3) = fcat s2 (fcat s1 s3).
Proof.
by case/and3P=> H1 H2 H3; rewrite -!fcatA // (@fcatC s1 s2). Qed.

Lemma fcatCA (s1 s2 s3 : fmap) :
  [&& disj s1 s2, disj s2 s3 & disj s1 s3] ->
  fcat (fcat s1 s2) s3 = fcat (fcat s1 s3) s2.
Proof.
by case/and3P=> H1H2 H3; rewrite !fcatA // ?(@fcatC s2 s3) ?(disjC s3). Qed.
...
by apply: cancel_ins H5; rewrite supp_fcat negb_or /= ?H1 ?H3 H. Qed.

Lemma fcatKs (s s1 s2 : fmap) :
  disj s s1 && disj s s2 -> fcat s s1 = fcat s s2 -> s1 = s2.
Proof.
case/andP=> H1 H2.
rewrite (fcatC H1) (fcatC H2); apply: fcatsK. by rewrite -!(disjC s) H1 H2. Qed.
...
case: ifP=> E; last by apply: IH.
  rewrite disj_ins supp_kfilt mem_filter negb_and H1 orbT /=. by apply: IH. Qed.

Lemma in_disj_kfilt p1 p2 s : {in supp s, forall x, ~~ p1 x || ~~ p2 x} ->
  disj (kfilter p1 s) (kfilter p2 s).
Proof.
elim/fmap_ind': s => [|k v s _ IH] //= H.
rewrite !kfilt_ins; case: ifP=> E1; case: ifP => E2.
-
  move: (H k); rewrite E1 E2 supp_ins inE /= eq_refl /=. by move/(_ (erefl _)).
-
...
by apply: IH => x S; apply: H; rewrite supp_ins inE /= S orbT.
by apply: IH => x S; apply: H; rewrite supp_ins inE /= S orbT. Qed.
...
by move=> H; apply: ReflectF; move/fmapE; move/H. Qed.
...
Lemma map_key_mapf (m : seq (K * U)) :
  map key (mapf' m) = map key m.
Proof.
by elim: m => [|[k v] m IH] //=; rewrite IH. Qed.
...
case=> E _; rewrite -{k2}E in S2 *.
  move/(IH (path_sorted S1) _ (path_sorted S2) _ S). by rewrite /disj /supp /= => ->. Qed.

Lemma mapf_disj s1 s2 s1' s2' : mapf s1 = mapf s2 -> mapf s1' = mapf s2' -> disj s1 s1' = disj s2 s2'.
Proof.
  move=> X1 X2; apply: eq_trans (mapf_disjL _ X1) _.
by rewrite !(disjC s2); apply: mapf_disjL X2. Qed.
...
Lemma foldf_ins g e k v f : path ord k (supp f) ->
...
Lemma sorted_mapk m : sorted ord (supp (mapk m)).
Proof.
case: (mapk m) => [s] I //=. Qed.
...
by rewrite /mapk foldf_ins // /supp /= seqof_ins //= H1 andbT (Hf H). Qed.

Lemma mapk_nil : mapk (nil A V) = nil B V.
Proof. by rewrite /mapk //=. Qed.

Lemma mapk_ins k v s : path ord k (supp s) -> mapk (ins k v s) = ins (f k) v (mapk s).
...
Proof.
by elim/fmap_ind': m => //k v s L IH; rewrite -{2}IH /mapk foldf_ins //. Qed.

Lemma map_comp m : mapk g (@mapk A B V f m) = mapk (comp g f) m.
Proof.
elim/fmap_ind': m => //= k v s P IH.
rewrite [mapk (g \o f) _]mapk_ins //.
  rewrite mapk_ins // mapk_ins //; first by rewrite IH. exact: (path_mapk Hf P). Qed.
...
Variable (assoc : forall x y z, obind (zip_f x) (zip_f y z) = obind (zip_f^~ z) (zip_f x y)).

Variable (unitL : forall x, zip_f (unit_f x) x = Some x).

Variable (unitE : forall x y, (exists z, zip_f x y = Some z) <-> unit_f x = unit_f y).

Fixpoint zip' (s1 s2 : seq (K * V)) :=
  match s1, s2 with [::], [::] => Some [::]
  | (k1, v1) :: s1', (k2, v2) :: s2' =>
        if k1 == k2 then
        if zip_f v1 v2 is Some v then
      if zip' s1' s2' is Some s' then Some ((k1, v) :: s') else None else None else None
  | _, _ => None
  end.
...
case=> [|[k2 v2] s2] //=; rewrite eq_sym; case: eqP => //->{k2}. by rewrite comm IH. Qed.
...
case E3: (zip_f v1 v2)=> [w3|//].
case S3: (zip' s1 s2)=> [t3|] //=; rewrite eq_refl.
  move: (IH s2 s3); rewrite /obind /oapp S1 S3 => <-. by case: (zip_f w3 v3).
rewrite eq_refl.
case E3: (zip_f v1 v2)=> [w3|]; move: (assoc v1 v2 v3); rewrite /obind /oapp E1 E3 => -> //=.
case S3: (zip' s1 s2)=> [t3|]; move: (IH s2 s3); rewrite /obind /oapp S3 S1 => -> /=.
- by rewrite eq_refl. by case: (zip_f w3 v3). Qed.

Lemma zip_unitL' s : zip' (zip_unit' s) s = Some s.
Proof.
by elim: s => [|[k v] s IH] //=; rewrite eq_refl unitL IH. Qed.

Lemma map_key_zip' s1 s2 s : zip' s1 s2 = Some s -> map key s = map key s1.
Proof.
elim: s1 s2 s => [|[k1 v1] s1 IH]; case=> [|[k2 v2] s2] //= s; first by case=> <-.
case: eqP => //->{k1}; case: (zip_f v1 v2)=> // w; case Z: (zip' s1 s2)=> [t|//].
  by case=> <-; rewrite -(IH _ _ Z). Qed.

Lemma zip_unitE' s1 s2 : (exists s, zip' s1 s2 = Some s) <-> zip_unit' s1 = zip_unit' s2.
Proof.
split.
-
...
by move/IH: E2=> ->; congr ((_, _):: _); apply/unitE; exists w.
  ...
  case=> k2 v2 s2 //= [<- {k2}]; case/unitE=> s ->; case/IH=> t ->. by exists ((k1, s) :: t); rewrite eq_refl. Qed.

Lemma zip_sorted' s1 s2 : sorted ord (map key s1) ->
...
Definition zip f1 f2 : option (finMap K V) :=
  match f1, f2 with FinMap s1 pf1, FinMap s2 pf2 =>
      match zip' s1 s2 as z return zip' s1 s2 = z -> _ with Some s => fun pf => Some (FinMap (zip_sorted' pf1 pf))
  | None => fun pf => None
    end (erefl _)
  end.

Lemma zip_unit_sorted' s : sorted ord (map key s) ->
  sorted ord (map key (zip_unit' s)).
Proof.
  rewrite (_ : map key s = map key (zip_unit' s)) //. by apply: (map_key_zip' (s2:= s)); apply: zip_unitL'. Qed.

Definition zip_unit f := let: FinMap s pf := f in FinMap (zip_unit_sorted' pf).

Lemma zip_unitE f1 f2 : (exists f, zip f1 f2 = Some f) <-> zip_unit f1 = zip_unit f2.
Proof.
case: f1 f2 => s1 H1 [s2 H2] /=; split.
-
case=> s; move: (zip_sorted' _).
case E: (zip' s1 s2)=> [t|//] _ _; apply/fmapE=> /=. by apply/zip_unitE'; exists t.
case; case/zip_unitE'=> s E; move/(zip_sorted' H1): (E)=> T.
exists (FinMap T); move: (zip_sorted' _); rewrite E=> pf. by congr Some; apply/fmapE. Qed.
...
case E: (zip_f v1 v2)=> [w|//]; case F: (zip' s1 s2)=> [t|//]. by move/IH: F => <- [<-]. Qed.

Lemma zip_supp f1 f2 f : zip f1 f2 = Some f -> supp f =i supp f1.
Proof.
case: f1 f2 f => s1 H1 [s2 H2] [s3 H3] /=; move: (zip_sorted' _).
case E: (zip' s1 s2)=> [t|//]; move=> pf [F x]; rewrite -{s3}F in H3 *. by rewrite /supp (zip_supp' E). Qed.

Lemma zip_filter' s1 s2 s x : zip' s1 s2 = Some s ->
  zip' (filter (predCk V x) s1) (filter (predCk V x) s2) = Some (filter (predCk V x) s).
Proof.
elim: s1 s2 s => [|[k1 v1] s1 IH]; case=> [|[k2 v2] s2] //= s; first by case=> <-.
case: eqP => //<- {k2}; case E1: (zip_f v1 v2)=> [a|//].
case E2: (zip' s1 s2)=> [t|//]; move/IH: E2 => E2 [<- {s}].
case: eqP => [-> {k1}|] /=; first by rewrite E2 eq_refl. by rewrite eq_refl E1 E2; case: eqP. Qed.

Lemma zip_rem f1 f2 f x : zip f1 f2 = Some f -> zip (rem x f1) (rem x f2) = Some (rem x f).
Proof.
case: f1 f2 f => s1 H1 [s2 H2] [s3 H3] /=; do 2![move: (zip_sorted' _)].
case E1: (zip' s1 s2)=> [t|//]; case E2: (zip' _ _)=> [q|]; rewrite (@zip_filter' _ _ _ _ E1) // in E2.
by case: E2 => <-{q}pf1 pf2 [E]; congr Some; apply/fmapE; rewrite /= E. Qed.

Lemma zip_fnd f1 f2 f x (v : V) :
  zip f1 f2 = Some f -> fnd x f = Some v ->
  exists v1, exists v2, [/\ zip_f v1 v2 = Some v, fnd x f1 = Some v1 & fnd x f2 = Some v2].
Proof.
case: f1 f2 f => s1 H1 [s2 H2] [s3 H3] /=; move: (zip_sorted' _).
...
by case: (filter (predk V x) t) (IH _ _ E2). Qed.

(* The other direction of the zip_fnd lemma *)

Lemma fnd_zip f1 f2 f x (v1 v2 : V) :
  fnd x f1 = Some v1 -> fnd x f2 = Some v2 -> zip f1 f2 = Some f -> fnd x f = zip_f v1 v2.
Proof.
...
case E2: (zip' s1 s2)=> [t|//] [<- {s}].
  case: eqP=> /=; last by case: eqP => // _ _; apply: IH. by move=> -> {k1}; rewrite eq_refl; case=> <- [<-]. Qed.

Lemma zunit0 : zip_unit (nil K V) = nil K V.
Proof. by apply/fmapE. Qed.

Lemma zunit_ins f k v :
  zip_unit (ins k v f) = ins k (unit_f v) (zip_unit f).
Proof.
...
Lemma zunit_fcat f1 f2 : zip_unit (fcat f1 f2) = fcat (zip_unit f1) (zip_unit f2).
Proof.
elim/fmap_ind': f2 f1=> [|k v f2 H IH] f1 /=.
-
rewrite fcats0; set j := FinMap _.
  by rewrite (_ : j = nil K V) ?fcat0s //; apply/fmapE.
by rewrite fcat_sins zunit_ins IH -fcat_sins zunit_ins. Qed.

Lemma zunit_supp f : supp (zip_unit f) = supp f.
Proof.
case: f => s H; rewrite /supp /= {H}.
by elim: s => [|[k v] s IH] //=; rewrite IH. Qed.

Lemma zunit_disj f1 f2 : disj f1 f2 = disj (zip_unit f1) (zip_unit f2).
Proof.
case: disjP; case: disjP=> //; rewrite !zunit_supp.
-
by move=> x H1 H2; move/(_ _ H1); rewrite H2. by move=> H x; move/H/negbTE=> ->. Qed.
