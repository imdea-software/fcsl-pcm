...
From mathcomp
Require Import ssrnat eqtype seq path fintype.
...
Structure mixin_of (T : eqType) :=
  Mixin {ordering : rel T; _ : irreflexive ordering;
...
Definition oleq (T : ordType) (t1 t2 : T) :=
  ord t1 t2 || (t1 == t2).
...
Proof.
by move=> E1 E2; move: (trans E1 E2); rewrite irr. Qed.
...
rewrite /oleq; move=> T1; case/orP; first by move/(trans T1)=> ->. by move/eqP=> <-; rewrite T1. Qed.

Lemma sorted_oleq s : sorted (@ord T) s -> sorted (@oleq T) s.
Proof.
by elim: s => [|x s IH] //=; apply: sub_path=> z y; rewrite /oleq=> ->. Qed.
...
by move: (total x y); rewrite H1 H2 H3. Qed.
...
Definition strictly_increasing f x y :=
  @ord A x y -> @ord B (f x) (f y).

Structure mono : Type :=
  Mono {fun_of : A -> B; _ : forall x y, strictly_increasing fun_of x y}.
...
Proof. by move=> x; rewrite /= ltnn. Qed.

Lemma trans_ltn_nat : transitive ltn.
Proof. by apply: ltn_trans. Qed.
...
case: ifP=> H3; last by apply: trans.
  by rewrite (eqP H3)=> R1; move/(nsym R1). Qed.
...
by move: (total x1 y1); rewrite H1. Qed.
...
by rewrite -(nth_index x H1) -(nth_index x H2) H eq_refl. Qed.
...
Fixpoint ords x : pred (seq T) :=
  ...
  | [::], t :: ts => true
  | x :: xs, y :: ys =>
               if x == y then ords xs ys else ord x y
  | _ :: _, [::] => false
  end.
...
case: eqP =>//[->|H1] H2; first by move/(nsym H2). by move/(trans H2). Qed.
...
by move=> _; case: (or3P (total x y))=> [-> // | /eqP/H1//|-> //]; rewrite orbT. Qed.
...
by elim: s => [|y' s IH]; rewrite /= inE // IH orbT. Qed.

Lemma path_last (A : ordType) (s : seq A) x : path oleq x s -> oleq x (last x s).
Proof.
  move/(order_path_min (@otrans _)); rewrite -nth_last. by case: s => // h s' /all_nthP->. Qed.
...
-
  apply: IH (path_sorted H) H1 _ => z H3; apply: H2. by rewrite inE /= H3 orbT.
apply/eqP; move: (H2 (last x s)) (path_last H1); rewrite inE /= /oleq eq_sym.
case: totalP=> //=; case E: (last x s \in s)=> //.
by move/negbT/seq_last_in: E => ->; rewrite irr. Qed.
